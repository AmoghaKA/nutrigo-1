...
export default function AlternativesPage() {
  ...
  const getPurchaseLink = (platformKey: string, alt: Alternative) => {
    // Force dynamic search based on the product type/category name
    // Strip parentheticals like " (Fox Nuts)" to improve search results
    const cleanName = alt.name.replace(/\s*\(.*?\)\s*/g, '').trim();
    const query = encodeURIComponent(cleanName);
    switch (platformKey) {
      case 'blinkit': return `https://blinkit.com/s/?q=${query}`;
      case 'zepto': return `https://www.zeptonow.com/search?query=${query}`;
      case 'swiggy': return `https://www.swiggy.com/instamart/search?query=${query}`;
                        ...
                        <DialogContent className="bg-slate-900/95 backdrop-blur-xl border border-emerald-500/20 max-w-[calc(100vw-2rem)] sm:max-w-lg mx-4">
                          ...
                          <div className="grid grid-cols-2 gap-3 sm:gap-4 py-4">
                            {quickCommercePlatforms.map((platform) => (
                              <a
                                key={platform.key}
                                href={getPurchaseLink(platform.key, alt)}
                                target="_blank"
                                rel="noopener noreferrer"
                                onClick={(e) => {
                                  e.preventDefault();
                                  window.open(getPurchaseLink(platform.key, alt), '_blank');
                                  setOpenDialogs({ ...openDialogs, [`buy-${i}`]: false });
                                }}
                                className="group"
                              >
